    ...
    <div style={{ maxWidth: '1080px', margin: '0 auto', padding: '36px 24px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
      ...
      <div className="glass-card" style={{ padding: '36px', border: '1px solid rgba(99, 102, 241, 0.4)', position: 'relative' }}>
        ...
        <div style={{ marginTop: '24px', padding: '14px', background: 'rgba(255, 255, 255, 0.02)', borderRadius: '10px', border: '1px solid var(--border-glass)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px', fontSize: '0.8rem' }}>
            <span style={{ fontWeight: 600, color: '#ffffff' }}>Profile Strength for AI Matching</span>
      ...
      {isEditing ? (
        ...
        <div className="glass-card" style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <h3 style={{ fontSize: '1.2rem', color: '#ffffff' }}>Edit Builder Details</h3>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '4px' }}>Full Name</label>
              <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} style={{ width: '100%', padding: '10px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '8px', color: '#ffffff', fontSize: '0.9rem' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '4px' }}>College</label>
              <input type="text" value={college} onChange={(e) => setCollege(e.target.value)} style={{ width: '100%', padding: '10px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '8px', color: '#ffffff', fontSize: '0.9rem' }} />
            </div>
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '4px' }}>Bio</label>
            <textarea rows={3} value={bio} onChange={(e) => setBio(e.target.value)} style={{ width: '100%', padding: '10px', background: 'rgba(255, 255, 255, 0.05)', border: '1px solid var(--border-glass)', borderRadius: '8px', color: '#ffffff', fontSize: '0.9rem', resize: 'vertical' }} />
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '6px' }}>Skills</label>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '8px' }}>
              {ALL_AVAILABLE_SKILLS.slice(0, 16).map(s => (
                <button type="button" key={s} onClick={() => toggleSkill(s)} style={{ padding: '4px 10px', borderRadius: '6px', background: skills.includes(s) ? 'rgba(99, 102, 241, 0.25)' : 'rgba(255,255,255,0.04)', color: skills.includes(s) ? '#ffffff' : 'var(--text-secondary)', border: '1px solid var(--border-glass)', fontSize: '0.75rem' }}>
                  {skills.includes(s) ? '✓ ' : '+ '}{s}
                </button>
              ))}
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <input type="text" value={customSkill} onChange={(e) => setCustomSkill(e.target.value)} placeholder="Add another skill..." style={{ flex: 1, padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-glass)', borderRadius: '6px', color: '#ffffff', fontSize: '0.8rem' }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '4px' }}>Hours/Week: {hoursPerWeek}h</label>
              <input type="range" min={5} max={40} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(parseInt(e.target.value))} style={{ width: '100%', accentColor: 'var(--accent-primary)' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: '#ffffff', marginBottom: '4px' }}>Portfolio URL</label>
              <input type="url" value={portfolioUrl} onChange={(e) => setPortfolioUrl(e.target.value)} placeholder="https://..." style={{ width: '100%', padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-glass)', borderRadius: '6px', color: '#ffffff', fontSize: '0.85rem' }} />
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div className="glass-card" style={{ padding: '28px' }}>
              <h3 style={{ fontSize: '1.1rem', color: '#ffffff', marginBottom: '10px' }}>About Me</h3>
            ...
            <div className="glass-card" style={{ padding: '28px' }}>
              <h3 style={{ fontSize: '1.1rem', color: '#ffffff', marginBottom: '14px' }}>Core Skills</h3>
            ...
            {currentUser.learning_goals && currentUser.learning_goals.length > 0 && (
              <div className="glass-card" style={{ padding: '28px' }}>
                <h3 style={{ fontSize: '1.1rem', color: '#ffffff', marginBottom: '14px' }}>Skills I Want to Learn</h3>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div className="glass-card" style={{ padding: '28px' }}>
              <h3 style={{ fontSize: '1.1rem', color: '#ffffff', marginBottom: '16px' }}>Availability & Schedule</h3>
            ...
            <div className="glass-card" style={{ padding: '28px' }}>
              <h3 style={{ fontSize: '1.1rem', color: '#ffffff', marginBottom: '14px' }}>My Active Projects ({userProjects.length})</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {userProjects.map(p => (
                  <div
                    key={p.id}
                    onClick={() => navigate(`/projects/${p.id}`)}
                    style={{ padding: '12px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '8px', border: '1px solid var(--border-glass)', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                  >
                    <div>
